...
const HELP: &[HelpEntry] = &[
    ...
    // Figure controls
    HelpEntry { name: "figure",   brief: "Reset figure state or switch to HTML output",
        detail: "figure()              — reset figure, return to TUI plotting\nfigure(\"file.html\")   — reset figure, switch to HTML output mode\n\nIn HTML mode, plot/xlabel/ylabel/title/grid/clf etc. write to the\nHTML file instead of the terminal. Refresh the browser to see updates.\nCall figure() with no args to return to normal TUI plotting.\n\nExamples:\n  figure(\"temp.html\")\n  plot(linspace(0,10,100), sin(linspace(0,10,100)))\n  xlabel(\"Time\")\n  figure()  % back to TUI" },
