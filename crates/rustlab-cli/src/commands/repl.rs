...
const HELP: &[HelpEntry] = &[
    ...
    HelpEntry { name: "savehist", brief: "DEPRECATED — use histogram(v); savefig(\"file.svg\")",
        detail: "savehist() is deprecated.  Use the interactive + savefig pattern instead:\n  histogram(v, 20)         — display histogram\n  savefig(\"file.svg\")       — save current figure to file\n\nLegacy signatures still work but emit a warning." },
    ...
    HelpEntry { name: "savefig",  brief: "Save a plot to PNG, SVG, or interactive HTML",
        detail: "savefig(\"file.html\")           — save current figure as interactive Plotly HTML\nsavefig(v, \"file.svg\")          — save line plot as SVG\nsavefig(v, \"file.png\", \"title\")  — save line plot as PNG with title\n\n  Extension determines format: .svg, .png, or .html\n  .html exports all subplots with interactive zoom/pan/hover (Plotly CDN)\n  v may be a vector or an n×1 column matrix." },
    HelpEntry { name: "savestem", brief: "DEPRECATED — use stem(v, \"title\"); savefig(\"file.svg\")",
        detail: "savestem() is deprecated.  Use the interactive + savefig pattern instead:\n  stem(v, \"title\")         — display stem plot\n  savefig(\"file.svg\")       — save current figure to file\n\nLegacy signatures still work but emit a warning." },
    HelpEntry { name: "savedb",   brief: "DEPRECATED — use plotdb(Hz, \"title\"); savefig(\"file.svg\")",
        detail: "savedb() is deprecated.  Use the interactive + savefig pattern instead:\n  plotdb(Hz, \"title\")       — display dB frequency response\n  savefig(\"file.svg\")       — save current figure to file\n\nLegacy signatures still work but emit a warning." },
    HelpEntry { name: "imagesc",  brief: "Display matrix as a colour heatmap in the terminal",
        detail: "imagesc(M)\nimagesc(M, colormap)\n  colormap: \"viridis\" (default), \"jet\", \"hot\", \"gray\"\n  Press any key to close." },
    HelpEntry { name: "savebar",     brief: "DEPRECATED — use bar(y, \"title\"); savefig(\"file.svg\")",
        detail: "savebar() is deprecated.  Use the interactive + savefig pattern instead:\n  bar(y, \"title\")           — display bar chart\n  savefig(\"file.svg\")       — save current figure to file\n\nLegacy signatures still work but emit a warning." },
    HelpEntry { name: "savescatter", brief: "DEPRECATED — use scatter(x, y, \"title\"); savefig(\"file.svg\")",
        detail: "savescatter() is deprecated.  Use the interactive + savefig pattern instead:\n  scatter(x, y, \"title\")    — display scatter plot\n  savefig(\"file.svg\")       — save current figure to file\n\nLegacy signatures still work but emit a warning." },
    HelpEntry { name: "saveimagesc", brief: "DEPRECATED — use imagesc(M); savefig(\"file.svg\")",
        detail: "saveimagesc() is deprecated.  Use the interactive + savefig pattern instead:\n  imagesc(M, \"jet\")         — display heatmap\n  savefig(\"file.svg\")       — save current figure to file\n\nLegacy signatures still work but emit a warning." },
    // Figure controls
...
fn print_help_list() {
    ...
    let categories = [
        ...
        ("Plotting",         &["plot","stem","bar","scatter","hline","yline","plotdb","imagesc",
                               "savefig","hist"]),
        ("Deprecated (use interactive + savefig)",
                             &["savestem","savebar","savescatter",
                               "savedb","saveimagesc","savehist"]),
