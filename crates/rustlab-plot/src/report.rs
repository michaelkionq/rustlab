    ...
    for (i, entry) in report.entries.iter().enumerate() {
        match entry {
            ReportEntry::Figure { title, state } => {
                ...
                let plotly_div = render_figure_plotly_div(state, &div_id);
...
<h1>{title}</h1>
{sections}
<footer style="color:#585b70; font-size:0.8rem; margin-top:2rem; padding-top:1rem; border-top:1px solid #313244;">
